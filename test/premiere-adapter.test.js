...
function makeFolder(name, parent, context) {
  const folder = {
    kind: "folder",
    name,
    parent,
    ...
    createBinAction(childName) {
      assert.equal(context.locked && context.transaction, true, "actions must be created inside lockedAccess/executeTransaction");
      return action(() => { this.items.push(makeFolder(childName, this, context)); });
    },
...
function makeFixture(options = {}) {
  ...
  const parent = makeFolder("Footage", null, context);
  const root = makeFolder("Root", null, context);
  ...
  const source = {
    ...
    async getMedia() { return { duration: Promise.resolve({ seconds: duration }) }; },
  ...
  const project = {
    name: "Test Project",
    guid: "project-guid",
    sequences: options.existingSequence ? [{ guid: "old", name: options.existingSequence }] : [],
    activeSequence: null,
    transactions: [],
    ...
    async setActiveSequence(sequence) { this.activeSequence = sequence; return true; },
    async closeSequence(sequence) { if (this.activeSequence === sequence) this.activeSequence = null; return true; },
    async deleteSequence(sequence) { this.sequences = this.sequences.filter((entry) => entry !== sequence); return true; },
  };
  ...
  return { ppro, project, source, parent, root };
}

test("inspects one supported selected clip", async () => {
...
test("creates hard-boundary subclips in an isolated bin and a new sequence", async () => {
  ...
  const generatedBin = parent.items.find((item) => item.kind === "folder" && item.name.startsWith("PAI_"));
...
test("rolls back generated bin and subclips when sequence creation fails", async () => {
  ...
  assert.equal(parent.items.some((item) => item.kind === "folder" && item.name.startsWith("PAI_")), false);
});
