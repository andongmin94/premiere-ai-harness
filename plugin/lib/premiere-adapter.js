(function (root, factory) {
  ...
  async function inspectSelection(ppro) {
    ...
    return Object.freeze({
      ...
      hasTranscript: Boolean(ppro.Transcript.hasTranscript(context.clip)),
    });
  }
  ...
  async function loadSelectedTranscript(ppro) {
    const context = await selectedClipContext(ppro);
    await assertSupportedClip(context.clip);
    if (!ppro.Transcript.hasTranscript(context.clip)) {
  ...
  async function createRoughCut(ppro, keepRanges, sequenceName, options) {
    ...
    const parentBin = context.clip.getParentBin();
    ...
    const operationId = `PAI_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
    ...
    try {
      ...
      await cleanupGenerated({ project, parentBin, runBin, subclips, sequence });
      throw error;
    }
  }
  ...
  async function readSourceTiming(clip) {
    ...
    const duration = Number(media && media.duration && media.duration.seconds);
    ...
    const frameRate = Number(interpretation && interpretation.getFrameRate && interpretation.getFrameRate());
  ...
  async function createRunBin(project, parentBin, name, ppro, delay) {
    executeActions(project, [parentBin.createBinAction(name, false)], "Premiere AI Harness: 작업 빈 생성");
  ...
  async function createSubclips(project, sourceClip, ranges, names, frameRateObject, ppro) {
    for (let offset = 0; offset < ranges.length; offset += ACTION_BATCH_SIZE) {
      ...
      for (let index = offset; index < Math.min(offset + ACTION_BATCH_SIZE, ranges.length); index += 1) {
        ...
        actions.push(sourceClip.createSubClipAction(names[index], start, end, true, { takeVideo: true, takeAudio: true }));
      }
  ...
  async function waitForNamedClipItems(folder, names, ppro, delay) {
    ...
    const result = await poll(async function () {
  ...
  async function moveItems(project, sourceFolder, targetFolder, items) {
    for (let offset = 0; offset < items.length; offset += ACTION_BATCH_SIZE) {
      const actions = items.slice(offset, offset + ACTION_BATCH_SIZE).map((item) => sourceFolder.createMoveItemAction(item, targetFolder));
      executeActions(project, actions, "Premiere AI Harness: 생성 서브클립 정리");
    }
  }

  function executeActions(project, actions, label) {
    let committed = false;
  ...
  async function cleanupGenerated(resources) {
    const project = resources.project;
    try {
      if (resources.sequence) {
        try { await project.closeSequence(resources.sequence); } catch (_) { /* best effort */ }
        try { await project.deleteSequence(resources.sequence); } catch (_) { /* best effort */ }
      }
      if (resources.runBin) {
        const items = await resources.runBin.getItems();
        if (items && items.length) executeActions(project, items.map((item) => resources.runBin.createRemoveItemAction(item)), "Premiere AI Harness: 실패 서브클립 정리");
        executeActions(project, [resources.parentBin.createRemoveItemAction(resources.runBin)], "Premiere AI Harness: 실패 작업 빈 정리");
      } else if (resources.subclips && resources.subclips.length) {
        executeActions(project, resources.subclips.map((item) => resources.parentBin.createRemoveItemAction(item)), "Premiere AI Harness: 실패 서브클립 정리");
      }
    } catch (_) {
      // Generated assets use a unique PAI_ prefix so they can be removed manually if Premiere rejects cleanup.
    }
  }

  async function poll(check, delay, failureMessage) {
