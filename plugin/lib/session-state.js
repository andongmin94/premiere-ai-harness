(function (root, factory) {
  ...
  function setTranscript(session, value) {
    ...
    if (!value || !Array.isArray(value.segments) || value.segments.length === 0) throw new Error("분석할 전사 구간이 없습니다.");
    session.transcript = Object.freeze({
      source: value.source === "premiere" ? "premiere" : "pasted",
      raw: value.raw == null ? null : String(value.raw),
    });
    session.segments = value.segments.slice();
    session.plan = null;
    return session;
  }
  ...
  function setPlan(session, plan) {
    ...
    if (!plan || !Array.isArray(plan.candidates) || !Array.isArray(plan.selectedIds)) throw new Error("편집안이 올바르지 않습니다.");
    session.plan = plan;
    return session;
  }
  ...
  function canApply(session, certified) {
    requireSession(session);
    return !session.busy && Boolean(certified) && Boolean(session.selection) && Boolean(session.plan);
  }
  ...
  function normalizeSelection(value) {
    ...
    if (!clipId || !Number.isFinite(duration) || duration <= 0 || !Number.isFinite(frameRate) || frameRate <= 0) {
  ...
  return { createSession, setSelection, setTranscript, setPlan, resetAnalysis, resetSession, setBusy, canApply, sameSelection };
});
